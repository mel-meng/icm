...
class Test(TestCase):
    ...
    def test_conveyance_curve3(self):
        df = pd.read_excel('./river/cross_section3.xlsx', 'xs_single')
        xs = df['offset'].values
        ys = df['Z'].values
        ...
        curve.to_csv('./river/cross_section3_curve.csv')
        check_df = pd.read_excel('./river/cross_section3.xlsx', 'icm_check')
    ...
    def test_conveyance_curve3_with_panel(self):
        df = pd.read_excel('./river/cross_section3_with_panel.xlsx', 'xs_single')
        xs = df['offset'].values
        ys = df['Z'].values
        ns = df['roughness_N'].values
        ps = df['new_panel'].values
        ...
        # curve['level'] = curve['level'] - min(curve['level'].values)
        # curve.to_csv('./river/cross_section3_curve.csv')
        check_df = pd.read_excel('./river/cross_section3_with_panel.xlsx', 'icm_check')
        check_df.index = check_df['level']
        curve.index = curve['level']
        for level in curve['level'].unique():
            for fld in ['area', 'wp', 'ws']:
    ...
    def test_conveyance_curve2(self):
        df = pd.read_excel('./river/xs_test2.xlsx', 'xs_single')
        xs = df['offset'].values
        ys = df['Z'].values
        ns = df['roughness_N'].values
        ps = df['new_panel'].values
        curve = conveyance.conveyance_curve(xs, ys, ns, ps)
        curve.to_csv('./river/xs_test2_curve.csv')
        check_df = pd.read_excel('./river/xs_test2.xlsx', 'icm_check')
    ...
    def test_conveyance_curve(self):
        df = pd.read_excel('./river/xs_test.xlsx', 'xs_single')
        xs = df['offset'].values
        ys = df['Z'].values
        ns = df['roughness_N'].values
        ps = df['new_panel'].values
        curve = conveyance.conveyance_curve(xs, ys, ns, ps)
        check_df = pd.read_excel('./river/xs_test.xlsx', 'icm_check')
        check_df.index = check_df['level']
        curve.index = curve['level']
    ...
    def test_calculate_conveyance(self):
        ...
        for level in [0, 2, 5, 8, 10, 11, 12, 15]:
            ...
            if not results.empty:
                ...
                # plt.show()
                # results.to_csv('./river/%s.csv' % level, index=False)
                data = conveyance.get_panel(results)
                # data.to_csv('./river/%s_panels.csv' % level, index=False)
                summary = {'level': level}
                for v in ['k', 'wp', 'ws', 'area']:
                    summary[v] = data[v].sum()
                con_list.append(summary)
        curve = pd.DataFrame(con_list)  # .to_csv('./river/xs_curve.csv')
        check_df = pd.read_excel('./river/xs_test.xlsx', 'icm_check')
        check_df.index = check_df['level']
        curve.index = curve['level']
        for level in [2, 5, 8, 10, 11, 12, 15]:
